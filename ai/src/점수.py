...
def main():
    ...
    # JSON 출력
    json.dump([{"p_id":item["p_id"],**{k:round(next(x["score"] for x in res[k] if x["p_id"]==item["p_id"]),2) for k in res}} for item in res["혼잡도우선"]],sys.stdout,ensure_ascii=False,indent=2)
