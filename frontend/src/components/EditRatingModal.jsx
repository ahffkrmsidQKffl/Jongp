...
const EditRatingModal = ({ initialScore, onSave, onCancel }) => {
  ...
  const handlePlus = () => {
    if (score < 5)
      setScore((prev) => Math.min(5, Math.round((prev + 0.5) * 10) / 10));
  };
  const handleMinus = () => {
    if (score > 0)
      setScore((prev) => Math.max(0, Math.round((prev - 0.5) * 10) / 10));
  };
  const handleSubmit = () => onSave(score);
    ...
    <div className="edit-rating-overlay">
      <div className="edit-rating-modal">
        ...
        <div className="edit-score-controls">
          <button className="edit-minus" onClick={handleMinus}>
            -
          </button>
          <span className="edit-score">{score.toFixed(1)}점</span>
          <button className="edit-plus" onClick={handlePlus}>
            +
          </button>
        </div>
